...
export function DisclosureWorkflow({ fileName, onBack }: DisclosureWorkflowProps) {
...
因此，亟需一种新的技术方案来解决上述问题，提高${technicalField}领域的技术水平。`
  ...
  const handleAIRewrite = () => {
    ...
    setTimeout(() => {
      ...
      const totalText = contentBlocks.filter(b => b.type === "text").map(b => b.content).join("")
      
      if (totalText.length < 100) {
        warnings.push({ type: "brief", message: "技术方案描述过于简略，建议补充更多技术细节" })
      }
      
      const imageBlocks = contentBlocks.filter(b => b.type === "image")
      if (imageBlocks.some(b => !b.imageUrl)) {
        warnings.push({ type: "image", message: "存在未上传图片的图片区块，请检查" })
      }

      if (totalText.includes("等") || totalText.includes("之类")) {
        warnings.push({ type: "unclear", message: "文中存在模糊表述（如'等'、'之类'），建议明确具体内容" })
      }
    ...
    <div className="flex h-full flex-col bg-background">
      ...
      <header className="flex items-center justify-between border-b border-border bg-card px-6 py-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-lg font-semibold text-foreground">专利交底书</h1>
            <p className="text-sm text-muted-foreground">基于文件：{fileName}</p>
          </div>
        </div>
      </header>
        ...
        <div className="mx-auto max-w-4xl">
          ...
          {step === 1 && (
            <div className="space-y-6">
              <div className="rounded-lg border border-border bg-card p-6">
                <h2 className="mb-6 text-xl font-semibold text-foreground">填写基本信息</h2>
                
                <div className="space-y-6">
                  {/* 发明名称 */}
                  <div>
                    <label className="mb-2 block text-sm font-medium text-foreground">发明名称 *</label>
                    <input
                      type="text"
                      value={inventionName}
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <label className="mb-2 block text-sm font-medium text-foreground">联系人 *</label>
                      <input
                        type="text"
                        value={contactPerson}
                    ...
                    <div>
                      <label className="mb-2 block text-sm font-medium text-foreground">所属单位 *</label>
                      <input
                        type="text"
                        value={organization}
                  ...
                  {/* 申请类型 */}
                  <div>
                    <label className="mb-2 block text-sm font-medium text-foreground">申请类型 *</label>
                      ...
                      {["发明", "实用新型"].map((type) => (
                        <label key={type} className="flex items-center gap-2 cursor-pointer">
                          <input
                            ...
                            onChange={() => setApplicationType(type as "发明" | "实用新型")}
                  ...
                  {/* 技术领域 */}
                  <div>
                    <label className="mb-2 block text-sm font-medium text-foreground">技术领域 *</label>
            ...
            <div className="space-y-6">
              <div className="rounded-lg border border-border bg-card p-6">
                <div className="mb-4 flex items-center justify-between">
                  <h2 className="text-xl font-semibold text-foreground">本发明技术背景</h2>
                  <Button
                    variant="outline"
                  ...
                  >
                    <RefreshCw className={cn("h-4 w-4", isGeneratingBackground && "animate-spin")} />
                    重新生成
                  </Button>
                </div>
                
                <p className="mb-4 text-sm text-muted-foreground">
                  基于发明名称"{inventionName}"和技术领域"{technicalField}"自动生成，您可以修改以下内容：
                </p>
          ...
          {step === 3 && (
            <div className="space-y-6">
              <div className="rounded-lg border border-border bg-card p-6">
                <div className="mb-4 flex items-center justify-between">
                  <h2 className="text-xl font-semibold text-foreground">本发明的技术方案</h2>
                  <Button
                    onClick={handleAIRewrite}
                    disabled={isRewriting}
                    className="gap-2"
                  >
                    <Sparkles className={cn("h-4 w-4", isRewriting && "animate-pulse")} />
                    {isRewriting ? "AI 处理中..." : "AI 优化"}
                  </Button>
                </div>
                
                <p className="mb-4 text-sm text-muted-foreground">
                  请详细描述您的技术方案，可以添加文字说明和配图。AI 将帮助您优化表述并识别专有词汇。
                </p>

                {/* 内容块列表 */}
                <div className="space-y-4">
                  {contentBlocks.map((block, index) => (
                    <div key={block.id} className="group relative rounded-lg border border-border bg-background p-4">
                      <div className="mb-2 flex items-center justify-between">
                        <span className="text-xs font-medium text-muted-foreground">
                          {block.type === "text" ? `文本块 ${index + 1}` : `图片 ${index + 1}`}
                        </span>
              ...
              {aiWarnings.length > 0 && (
                <div className="rounded-lg border border-amber-500/50 bg-amber-500/10 p-4">
                  <div className="mb-2 flex items-center gap-2">
                    ...
                    <h3 className="font-medium text-amber-700 dark:text-amber-400">AI 检测到以下问题</h3>
                  </div>
            ...
            <div className="space-y-6">
              {isGeneratingEffects ? (
                ...
                <>
                  {/* 有益效果 */}
                  <div className="rounded-lg border border-border bg-card p-6">
                    <h2 className="mb-4 text-xl font-semibold text-foreground">本发明技术方案带来的有益效果</h2>
                    <p className="mb-4 text-sm text-muted-foreground">
                  ...
                  {/* 技术关键点与保护点 */}
                  <div className="rounded-lg border border-border bg-card p-6">
                    <h2 className="mb-4 text-xl font-semibold text-foreground">本发明的技术关键点和欲保护点</h2>
                    <p className="mb-4 text-sm text-muted-foreground">
            ...
            <div className="space-y-6">
              <div className="rounded-lg border border-border bg-card p-6">
                <div className="mb-6 flex items-center justify-between">
                  <h2 className="text-xl font-semibold text-foreground">专利交底书预览</h2>
                ...
                <div className="space-y-6">
                  ...
                  <div className="rounded-lg border border-border bg-background p-4">
                    ...
                    <div className="space-y-2 text-sm">
                      <p><span className="text-muted-foreground">发明名称：</span>{inventionName}</p>
                      <p><span className="text-muted-foreground">联系人：</span>{contactPerson}</p>
                      <p><span className="text-muted-foreground">所属单位：</span>{organization}</p>
                      <p><span className="text-muted-foreground">申请类型：</span>{applicationType}</p>
                      <p><span className="text-muted-foreground">技术领域：</span>本发明创造技术方案所属技术领域为{technicalField}</p>
                    </div>
                  </div>

                  {/* 2. 技术背景 */}
                  <div className="rounded-lg border border-border bg-background p-4">
                    <h3 className="mb-3 font-semibold text-foreground">二、本发明技术背景</h3>
                  ...
                  {/* 3. 技术方案 */}
                  <div className="rounded-lg border border-border bg-background p-4">
                    <h3 className="mb-3 font-semibold text-foreground">三、本发明的技术方案</h3>
                    <div className="space-y-4">
                      {contentBlocks.map((block, index) => (
                        <div key={block.id}>
                          {block.type === "text" ? (
                            ...
                            <div className="space-y-2">
                              <img
                                ...
                                className="max-h-64 rounded-lg object-contain"
                              />
                              <p className="text-sm text-muted-foreground">图 {index + 1}：{block.content}</p>
                            </div>
                          ) : null}
                        </div>
                      ))}
                    ...
                    {keywords.length > 0 && (
                      <div className="mt-6">
                        <h4 className="mb-2 text-sm font-medium text-foreground">关键词表</h4>
                        <div className="overflow-hidden rounded border border-border">
                          <table className="w-full text-sm">
                            <thead className="bg-accent/50">
                              <tr>
                                <th className="border-b border-border px-3 py-1.5 text-left font-medium">术语</th>
                                <th className="border-b border-border px-3 py-1.5 text-left font-medium">释义</th>
                              </tr>
                            </thead>
                            <tbody>
                              {keywords.map((kw, index) => (
                                <tr key={index} className="border-b border-border">
                                  ...
                                  <td className="px-3 py-1.5 text-muted-foreground">{kw.definition}</td>
                                </tr>
                              ))}
                            </tbody>
                          </table>
                  ...
                  {/* 4. 有益效果 */}
                  <div className="rounded-lg border border-border bg-background p-4">
                    <h3 className="mb-3 font-semibold text-foreground">四、本发明技术方案带来的有益效果</h3>
                    <div className="whitespace-pre-wrap text-sm text-foreground">
                      {beneficialEffects}
                    </div>
                  </div>
                  ...
                  {/* 5. 保护点 */}
                  <div className="rounded-lg border border-border bg-background p-4">
                    <h3 className="mb-3 font-semibold text-foreground">五、本发明的技术关键点和欲保护点</h3>
                    <div className="whitespace-pre-wrap text-sm text-foreground">
                      {protectionPoints}
                    </div>
                  </div>
      ...
      {step < 5 && (
        <footer className="flex items-center justify-between border-t border-border bg-card px-6 py-4">
          <div>
            {step > 1 && (
              <Button variant="outline" onClick={() => setStep((step - 1) as any)} className="gap-2 bg-transparent">
                <ArrowLeft className="h-4 w-4" />
                上一步
              </Button>
            )}
          ...
          <div className="flex gap-2">
            {step < 4 && (
              <Button
                ...
                disabled={
                  (step === 1 && (!inventionName.trim() || !contactPerson.trim() || !organization.trim() || !applicationType || !technicalField.trim())) ||
                  (step === 2 && !techBackground.trim()) ||
                  (step === 3 && contentBlocks.every(b => b.type === "text" ? !b.content.trim() : !b.imageUrl))
                }
                className="gap-2"
              >
                下一步
