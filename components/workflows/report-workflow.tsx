...
export function ReportWorkflow({ fileName, onBack }: ReportWorkflowProps) {
  ...
  const [selectedTemplate, setSelectedTemplate] = useState<string | null>("ipc-keywords")
  ...
  const [originalSearchResults, setOriginalSearchResults] = useState<PatentItem[]>([])
  ...
  const [authorizationProspect, setAuthorizationProspect] = useState<"高" | "中" | "低" | "无" | "">("")
  const [proposalGrade, setProposalGrade] = useState<"A" | "B" | "C" | "不通过" | "">("")
  const [conclusion, setConclusion] = useState("")
  const [enforcementability, setEnforcementability] = useState<"高" | "低" | "无" | "">("")
  ...
  const addKeyword = (word: string) => {
    if (word.trim() && !keywords.find(kw => kw.word === word.trim())) {
      ...
      const suggestions = (keywordSuggestions[word.trim()] || []).slice(0, 5).filter(
        w => !keywords.find(kw => kw.word === w)
      )
  ...
  useEffect(() => {
    if (usageProspect && authorizationProspect && proposalGrade) {
      const conclusionText = `根据专利检索分析，本提案${proposalName}的综合评估如下：\n\n` +
  ...
  }, [usageProspect, authorizationProspect, proposalGrade, standardAdaptation, vehicleApplication, proposalName, searchResults])
    ...
    <div className="flex h-screen flex-col">
      ...
      <header className="flex items-center justify-between border-b border-border bg-card px-6 py-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg font-semibold text-foreground">专利检索报告</h1>
      ...
      <main className="flex-1 overflow-y-auto p-6">
        <div className="mx-auto max-w-4xl space-y-6">
          ...
          {step === 2 && (
            <div className="space-y-6">
              <div className="rounded-lg border border-border bg-card p-6">
                <h2 className="mb-4 text-xl font-semibold text-foreground">选择检索式模版</h2>
                <div className="space-y-3">
              ...
              {generatedFormula && (
                <div className="rounded-lg border border-primary bg-primary/5 p-6">
                  <div className="mb-3 flex items-center justify-between">
                    <h3 className="font-semibold text-foreground">生成的检索式</h3>
                    <Button variant="outline" size="sm" onClick={copyFormula} className="gap-2 bg-transparent">
                      <FileText className="h-4 w-4" />
                      复制
                    </Button>
                  </div>
                  <div className="rounded-lg bg-background p-4">
                    <code className="text-sm text-foreground">{generatedFormula}</code>
                  </div>
                </div>
              )}
            </div>
            ...
            <div className="space-y-6">
              <div className="rounded-lg border border-border bg-card p-6">
                <div className="mb-4 flex items-center justify-between">
                  <h2 className="text-xl font-semibold text-foreground">检索专利文献</h2>
                      ...
                      <table className="w-full">
                        ...
                        <tbody>
                          {searchResults.map((patent) => (
                            <tr key={patent.id} className="border-b border-border hover:bg-accent/30 transition-colors">
                              ...
                              <td className="px-4 py-4 text-center">
                                <span className={cn(
                                  ...
                                  patent.category === "X" && "bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400",
                                  patent.category === "Y" && "bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400",
                                  patent.category === "A" && "bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400"
                                )}>
                                  {patent.category}
                                </span>
                              </td>
                              <td className="px-4 py-4">
                                <div className="flex items-center justify-center gap-2">
                                  ...
                                  <Button
                                    ...
                                    size="sm"
                                    onClick={() => {
                                      setSearchResults(searchResults.filter(p => p.id !== patent.id))
                                    }}
          ...
          {step === 4 && (
            <div className="space-y-6">
              {!reportGenerated ? (
                <div className="rounded-lg border border-border bg-card p-6">
                  <h2 className="mb-4 text-xl font-semibold text-foreground">完善报告信息</h2>
                  ...
                  <div className="space-y-6">
                    {/* 提案名称 */}
                    <div>
                      <label className="mb-2 block text-sm font-medium text-foreground">提案名称</label>
                      <input
                        type="text"
                        value={proposalName}
                    ...
                    <div className="flex gap-6">
                      <label className="flex items-center gap-2 cursor-pointer">
                        <input
                          ...
                          onChange={(e) => setStandardAdaptation(e.target.checked)}
                          className="h-5 w-5 rounded border-border text-primary focus:ring-2 focus:ring-primary"
                        />
                        <span className="text-sm font-medium text-foreground">标准适配</span>
                      </label>
                      <label className="flex items-center gap-2 cursor-pointer">
                        <input
                          type="checkbox"
                          checked={vehicleApplication}
                          onChange={(e) => setVehicleApplication(e.target.checked)}
                          className="h-5 w-5 rounded border-border text-primary focus:ring-2 focus:ring-primary"
                        />
                        <span className="text-sm font-medium text-foreground">车型应用</span>
                      </label>
                    </div>

                    {/* 用途前景 */}
                    <div>
                      <label className="mb-2 block text-sm font-medium text-foreground">用途前景</label>
                    ...
                    {/* 授权前景 */}
                    <div>
                      <label className="mb-2 block text-sm font-medium text-foreground">授权前景</label>
                      <div className="flex gap-3">
                        {["高", "中", "低", "无"].map((option) => (
                          <button
                            key={option}
                            onClick={() => setAuthorizationProspect(option as any)}
                            className={cn(
                              "flex-1 rounded-lg border px-4 py-2 text-sm font-medium transition-all",
                    ...
                    {/* 提案等级 */}
                    <div>
                      <label className="mb-2 block text-sm font-medium text-foreground">提案等级</label>
                      <div className="flex gap-3">
                    ...
                    {/* 结论 */}
                    <div>
                      <label className="mb-2 block text-sm font-medium text-foreground">结论</label>
                      <textarea
                        value={conclusion}
                ...
                <div className="rounded-lg border border-primary bg-primary/5 p-6">
                  <div className="mb-6 flex items-center gap-3">
                    ...
                    <div>
                      <h2 className="text-xl font-semibold text-foreground">检索报告已生成</h2>
                      <p className="text-sm text-muted-foreground">您可以下载报告或返回主界面</p>
                    </div>
                  </div>
                  <div className="space-y-4">
                    {/* 1. 提案名称 */}
                    <div className="rounded-lg border border-border bg-background p-4">
                      <h3 className="mb-2 font-semibold text-foreground">提案名称</h3>
                      <p className="text-sm text-foreground">{proposalName}</p>
                    </div>

                    {/* 2. 检索日期 */}
                    <div className="rounded-lg border border-border bg-background p-4">
                      <h3 className="mb-2 font-semibold text-foreground">检索日期</h3>
                    ...
                    {/* 3. 检索领域 */}
                    <div className="rounded-lg border border-border bg-background p-4">
                      <h3 className="mb-2 font-semibold text-foreground">检索领域</h3>
                      <div className="space-y-1 text-sm text-foreground">
                        <p>IPC/CPC 分类号：{ipcList.map(ipc => ipc.code).join(", ")}</p>
                    ...
                    {/* 4. 检索式 */}
                    <div className="rounded-lg border border-border bg-background p-4">
                      <h3 className="mb-2 font-semibold text-foreground">检索式</h3>
                    ...
                    {/* 5. 相关文件 */}
                    <div className="rounded-lg border border-border bg-background p-4">
                      <h3 className="mb-3 font-semibold text-foreground">相关文件（共 {searchResults.length} 件）</h3>
                      <div className="overflow-hidden rounded-lg border border-border">
                        <table className="w-full">
                          <thead className="bg-accent/50">
                            <tr>
        ...
        <div className="flex gap-2">
          {step < 4 && (
            <Button
              ...
              disabled={
                (step === 1 && (ipcList.length === 0 || keywords.length === 0)) ||
